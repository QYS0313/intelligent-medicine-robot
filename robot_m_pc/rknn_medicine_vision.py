...
class RknnYoloDetector:
    ...
    def start(self) -> None:
        ...
        result = self.rknn.init_runtime(core_mask=RKNNLite.NPU_CORE_AUTO)
        if result != 0:
...
def draw_detections(
    ...
    output = image.copy()
    visible_detections = list(detections)
...
def choose_slot_target(
    ...
    expected_count = 9 if slot_index <= 9 else 3
    if len(detections) >= expected_count:
        candidates = sorted(detections, key=lambda item: item.score, reverse=True)[:expected_count]
        if slot_index <= 9:
            by_y = sorted(candidates, key=lambda item: item.center[1])
            ordered: List[Detection] = []
            for start in range(0, 9, 3):
                ordered.extend(sorted(by_y[start : start + 3], key=lambda item: item.center[0]))
            return ordered[slot_index - 1]

        ordered = sorted(candidates, key=lambda item: item.center[0])
        return ordered[slot_index - 10]

    # Visual failure must never block the fixed-coordinate picking workflow.
    return choose_center_target(detections, image_width, image_height)


def choose_center_target(
    detections: Sequence[Detection],
    image_width: int,
    image_height: int,
) -> Optional[Detection]:
    ...
    return min(
        detections,
        key=lambda detection: squared_distance(detection.center, image_center),
    )
...
def main() -> None:
    ...
    with RknnYoloDetector(model_path, args.confidence, args.iou) as detector:
        with RealSenseCamera(
            ...
            while True:
                ...
                if (
                    ...
                    saved = cv2.imwrite(str(image_path), display)
                    if saved:
                        write_capture_result(
                            capture_request_id,
                            selected_slots[0],
