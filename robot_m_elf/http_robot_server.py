...
from collection_status import LATEST_IMAGE as COLLECTION_LATEST_IMAGE
from collection_status import clear_collection, read_status, start_collection
...
from robot_task_status import read_task_status, update_task_status
...
def pick_slots(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    clear_capture_images()
    collection_task = start_collection(len(slot_indices))
    update_task_status(
        "starting",
        "正在准备抓药任务",
        current_slot=slot_indices[0],
        current_index=1,
        total=len(slot_indices),
    ...
    try:
        with RobotArmSerial(SerialConfig(port=serial_port, baudrate=baudrate)) as arm:
            ...
            for index, slot_index in enumerate(slot_indices):
                ...
                # The overlay is only for the pre-pick still image. Once the
                # physical pick starts, the live vision view must show no box.
                write_selected_slots([])
        ...
        update_task_status(
            ...
            "抓药任务已完成，等待收药计数确认",
            current_slot=None,
            current_index=len(slot_indices),
            total=len(slot_indices),
        )
...
class RobotHttpHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path == "/vision/selection":
            ...
            return
        if self.path == "/collection/status":
            self.write_json(200, {"ok": True, "data": read_status()})
            return
        if self.path == "/task/status":
            self.write_json(200, {"ok": True, "data": read_task_status()})
            return
        if self.path == "/collection/latest.jpg":
            if not COLLECTION_LATEST_IMAGE.is_file():
                self.write_json(404, {"ok": False, "error": "collection image not found"})
                return
            self.write_bytes(200, COLLECTION_LATEST_IMAGE.read_bytes(), "image/jpeg")
            return
    ...
    def do_POST(self) -> None:
        try:
            ...
            if self.path == "/vision/selection":
                ...
                return
            if self.path == "/collection/reset":
                self.write_json(200, {"ok": True, "data": clear_collection()})
                return
            self.write_json(404, {"ok": False, "error": "not found"})
        except Exception as exc:
